...
class successView(views.APIView):
  ...
  def post(self, request):
    ...
    org.save()
    lnk = os.getenv("FRONTEND_URL")
    return HttpResponseRedirect(redirect_to=f'{lnk}app/payment-success/')
...
class PaymentFailView(views.APIView):
  ...
  def post(self, request):
    lnk = os.getenv("FRONTEND_URL")
    return HttpResponseRedirect(redirect_to=f'{lnk}app/payment-failed/')
...
class OrganizationRegisterView(viewsets.ModelViewSet):
  ...
  def create(self, request, *args, **kwargs):
    ...
    if serializer.is_valid():
      ...
      lnk = os.getenv("FRONTEND_URL")
      link = f"{lnk}api/organization/register/"
  ...
  def post(self, request):
    ...
    try:
      ...
      try:
        ...
        org_name = urlsafe_base64_encode(force_bytes(organization_name))
        lnk = os.getenv("FRONTEND_URL")
        link = f"{lnk}api/organization/add-user/"
